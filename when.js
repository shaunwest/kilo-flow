...
jack2d('WhenObject', ['helper', 'chrono', 'obj', 'Proxy'], function(Helper, Chrono, Obj, Proxy) {
  'use strict';
  ...
  function augmentMethods(targetObject) {
    var proxyObject = {};
    Object.keys(targetObject).forEach(function(prop) {
      if(!Helper.isFunction(targetObject[prop])) {
        return;
      }
      //proxyObject[prop] = augmentMethod(targetObject[prop]);
      proxyObject[prop] = augmentMethod(prop);
    });
    return proxyObject;
  }
  ...
  function augmentMethod(method) {
    return function() {
      ...
      ops.push({func: method, args: arguments});
      return this;
    };
  }

  function WhenObject(targetObject) {
    var augmentedObject = augmentMethods(targetObject);
    var flowObject = Obj.mixin([conditionals, operations, augmentedObject]);
    return flowObject.setFlowContext(targetObject);
  }
  ...
  var conditionals = {
    setFlowContext: function(context) {
      ...
      Chrono.register(Helper.call(this, function(deltaSeconds) {
        var i, len, flowConditions = flowObject.flowConditions;
        if(flowConditions) {
          for(i = 0, len = flowConditions.length; i < len; i++) {
            if(context[flowConditions[i]]) { // these are 'OR' conditions
              update(context, flowList);
              break;
            }
          }
        } else {
